...
// void main(){
  ...
  // for(int i = 1; i <= +radius; i++){
    ...
    // norm += coeff * 2.0;
    // blur += texture(tex, posn, (gl_FragCoord.xy + dir * i) * wh_inv) * coeff;
    // blur += texture(tex, posn, (gl_FragCoord.xy - dir * i) * wh_inv) * coeff;
  // }
...
void main(){
  ...
  blur += texture(tex, gl_FragCoord.xy * wh_inv) * coeff.x;
  norm += coeff.x;
  coeff.xy *= coeff.yz;  
  
  for(int i = 1; i <= radius; i++){
    blur += texture(tex, (gl_FragCoord.xy + dir * i) * wh_inv) * coeff.x;
    blur += texture(tex, (gl_FragCoord.xy - dir * i) * wh_inv) * coeff.x;
    norm += coeff.x * 2.0;
    coeff.xy *= coeff.yz;  
  }
