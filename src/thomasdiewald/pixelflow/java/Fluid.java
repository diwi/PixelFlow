...
import thomasdiewald.pixelflow.java.dwgl.DwGLTexture.TexturePingPong;;
...
public class Fluid{
  ...
  public Fluid(PixelFlow context, int viewport_width, int viewport_height, int fluidgrid_scale){
    this.context = context;
    context.papplet.registerMethod("dispose", this);
    
    shader_obstacleBounds        = context.createShader(PixelFlow.SHADER_DIR+"Fluid/obstacleBounds.frag");
    shader_advect                = context.createShader(PixelFlow.SHADER_DIR+"Fluid/advect.frag"        ); 
    shader_buoyancy              = context.createShader(PixelFlow.SHADER_DIR+"Fluid/buoyancy.frag"      ); 
    shader_divergence            = context.createShader(PixelFlow.SHADER_DIR+"Fluid/divergence.frag"    ); 
    shader_jacobi                = context.createShader(PixelFlow.SHADER_DIR+"Fluid/jacobi.frag"        ); 
    shader_gradient              = context.createShader(PixelFlow.SHADER_DIR+"Fluid/gradient.frag"      ); 
    shader_vorticityCurl         = context.createShader(PixelFlow.SHADER_DIR+"Fluid/vorticityCurl.frag" ); 
    shader_vorticityForce        = context.createShader(PixelFlow.SHADER_DIR+"Fluid/vorticityForce.frag");
    shader_renderFluid           = context.createShader(PixelFlow.SHADER_DIR+"Fluid/renderFluid.frag");
    shader_renderVelocityStreams = context.createShader(PixelFlow.SHADER_DIR+"Fluid/renderVelocityStreams.vert", PixelFlow.SHADER_DIR+"Fluid/renderVelocityStreams.frag");
   
    
    shader_addVelocityBlob       = context.createShader(PixelFlow.SHADER_DIR+"addData/addVelocityBlob.frag"   );
    shader_addDensityBlob        = context.createShader(PixelFlow.SHADER_DIR+"addData/addDensityBlob.frag"    );
    shader_addTemperatureBlob    = context.createShader(PixelFlow.SHADER_DIR+"addData/addTemperatureBlob.frag");
    shader_addDensityTexture     = context.createShader(PixelFlow.SHADER_DIR+"addData/addDensityTexture.frag" );
    shader_addObstacleTexture    = context.createShader(PixelFlow.SHADER_DIR+"addData/addObstacleTexture.frag");
...
//    shader_gradient.uniform1f     ("halfrdx"      , 0.5f / param.gridscale);
  ...
  public void renderFluidVectors(PGraphics2D dst, int spacing){
    ...
    int   lines_x    = (int) Math.round(w / spacing);
    int   lines_y    = (int) Math.round(h / spacing);
