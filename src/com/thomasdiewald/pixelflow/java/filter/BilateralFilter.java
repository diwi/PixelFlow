...
public class BilateralFilter {
  ...
  public BilateralFilter(PixelFlow context){
    this.context = context;
  }

  public void apply(PGraphics2D src, PGraphics2D dst, int radius, float sigma_color, float sigma_space) {
    if(src == dst){
  ...
  private void apply(int tex_handle, int w, int h, int radius, float sigma_color, float sigma_space){
    if(shader == null) shader = context.createShader(PixelFlow.SHADER_DIR+"Filter/bilateral.frag");
    shader.begin();
