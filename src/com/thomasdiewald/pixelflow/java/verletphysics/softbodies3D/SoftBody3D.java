...
//    createPolyhedronShape(shape, tetrahedron.faces, tetrahedron.vertices, particle.rad);
  ...
  private float[] cross = new float[3];
...
//      cross(v0, v1, v2, cross);
//      shape.normal(cross[0], cross[1], cross[2]); 
  ...
  public int cross(float[] v0,float[] v1, float[] v2, float[] cross){
  ...
  public void drawSprings(PGraphics pg, SpringConstraint3D.TYPE type, int display_mode){
    ...
    if(type == null) return;
    if(type == SpringConstraint3D.TYPE.STRUCT && !DISPLAY_SPRINGS_STRUCT) return;
    if(type == SpringConstraint3D.TYPE.SHEAR  && !DISPLAY_SPRINGS_SHEAR) return;
    if(type == SpringConstraint3D.TYPE.BEND   && !DISPLAY_SPRINGS_BEND) return;
