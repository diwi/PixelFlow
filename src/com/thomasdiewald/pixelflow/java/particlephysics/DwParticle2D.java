...
public class DwParticle2D implements DwCollisionObject{
  ...
  public DwSpringConstraint2D[] springs = null;
  ...
  protected void addSpring(DwSpringConstraint2D spring){
    ...
    if(springs == null || spring_count >= springs.length){
      ...
      if( springs == null){
        springs = new DwSpringConstraint2D[new_len];
      } else {
...
//    }
  ...
  protected DwSpringConstraint2D removeSpring(DwParticle2D pb){
    DwSpringConstraint2D removed = null;
    int pos = 0;
  ...
  public void updateSprings(DwParticle2D[] particles){
    // sum up force of attached springs
    DwParticle2D pa = this;
    for(int i = 0; i < spring_count; i++){
      DwSpringConstraint2D spring = springs[i];
...
//    if ((cy + rad) > ymax) { cy = ymax - rad; px = cx; py = cy; }
  ...
  private DwParticle2D collision_ptr = null;
