...
void main(){
  ...
  vec4 mA = GET_MIN(A0, A1, A2, A3);
  vec4 mB = GET_MIN(B0, B1, B2, B3);
  vec4 mC = GET_MIN(C0, C1, C2, C3);
  vec4 mD = GET_MIN(D0, D1, D2, D3);
  
  out_frag = GET_MIN(mA, mB, mC, mD);
#endif
