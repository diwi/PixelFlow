...
public class OpticalFlow {
  ...
  public OpticalFlow(PixelFlow context, int w, int h){
    this.context = context;
    context.papplet.registerMethod("dispose", this);
    
    shader_OF_gray                  = context.createShader(PixelFlow.SHADER_DIR+"OpticalFlow/OpticalFlowGray.frag");                                                                     
    shader_OF_rgba                  = context.createShader(PixelFlow.SHADER_DIR+"OpticalFlow/OpticalFlow.frag");                                                                         
    shader_OF_renderVelocity        = context.createShader(PixelFlow.SHADER_DIR+"OpticalFlow/renderVelocityShading.frag");                                                               
    shader_OF_renderVelocityStreams = context.createShader(PixelFlow.SHADER_DIR+"OpticalFlow/renderVelocityStreams.vert", PixelFlow.SHADER_DIR+"OpticalFlow/renderVelocityStreams.frag");
  ...
  public void update(PGraphics2D pg_curr) {
    ...
    // 1) copy/grayscale
    if(param.grayscale){
      Filter.get(context).luminance.apply(pg_curr, frameCurr.frame);
    } else {
      Filter.get(context).copy.apply(pg_curr, frameCurr.frame);
    }
    
    // 2) blur
    Filter.get(context).gaussblur.apply(frameCurr.frame, frameCurr.frame, frameCurr.tmp, param.blur_input);

    // 3) gradients
    Filter.get(context).sobel.apply(frameCurr.frame, frameCurr.sobelH, SobelFilter.DIR.HORZ_3x3);
    Filter.get(context).sobel.apply(frameCurr.frame, frameCurr.sobelV, SobelFilter.DIR.VERT_3x3);
    ...
    Filter.get(context).gaussblur.apply(frameCurr.velocity, frameCurr.velocity, frameCurr.tmp, param.blur_flow);
    
    // 6) mix with previous velocity
    Filter.get(context).mix.apply(framePrev.velocity, frameCurr.velocity, frameCurr.velocity, param.temporal_smoothing);
  ...
  public static class Frame{
    ...
    public void resize(PixelFlow context_, int w, int h, boolean grayscale){
