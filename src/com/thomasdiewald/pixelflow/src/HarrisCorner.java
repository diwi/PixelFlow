...
public class HarrisCorner {
  ...
  public HarrisCorner(PixelFlow context, int w, int h){
  ...
  public void resize(int w, int h) {
    ...
    shader_harrisMatrix = context.createShader(PixelFlow.SHADER_DIR+"HarrisCornerDetection/harrisMatrix.frag");
    shader_harrisCorner = context.createShader(PixelFlow.SHADER_DIR+"HarrisCornerDetection/harrisCorner.frag");
    shader_maximumSupp  = context.createShader(PixelFlow.SHADER_DIR+"Filter/LocalNonMaxSuppression.frag");
    shader_render       = context.createShader(PixelFlow.SHADER_DIR+"HarrisCornerDetection/harrisRender.frag");
  ...
  public void update(PGraphics2D pg_curr) {
    ...
    // 5) blur harrisMatrix
    Filter.get(context).gaussblur.apply(frameCurr.harrisMatrix, frameCurr.harrisMatrix, frameCurr.tmp, param.blur_harris);
    ...
    if(param.nonMaxSuppression){
      ...
      // copy back
      Filter.get(context).copy.apply(frameCurr.tmp, frameCurr.harrisCorner);
    }
    
    Filter.get(context).gaussblur.apply(frameCurr.harrisCorner, frameCurr.harrisCorner, frameCurr.tmp, param.blur_final);
  ...
  public static class Frame{
    ...
    public void resize(PixelFlow context_, int w, int h){
