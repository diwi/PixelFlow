...
//    SpringConstraint.makeAllSpringsBidirectional(physics.getParticles());
  ...
  DwParticle3D particle_nearest = null;
  
  // this transforms a coordinate (vec4) from model-space to screen-space
  public void transformToScreen(DwParticle3D particle, float[] buf_world, float[] buf_screen){
...
//      }
  ...
  public void repairAllSprings(){
    DwSpringConstraint3D.makeAllSpringsUnidirectional(physics.getParticles());
    for(DwSoftBody3D body : softbodies){
      for(DwParticle3D pa : body.particles){
