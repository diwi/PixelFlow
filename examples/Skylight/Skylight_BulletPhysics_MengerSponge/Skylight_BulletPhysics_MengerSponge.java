...
package Skylight.Skylight_BulletPhysics_MengerSponge;
...
public class Skylight_BulletPhysics_MengerSponge extends PApplet {
  ...
  public int     DISPLAY_WIREFRAME = 0;
...
//    String txt_model          = String.format(Locale.ENGLISH, "[1-9] model: %d", BUILDING);
    ...
    String txt_wireframe      = String.format(Locale.ENGLISH, "[1-3] wireframe: %d", DISPLAY_WIREFRAME);
...
//    text(txt_model          , tx, ty+=sy);
  ...
  public void keyReleased(){
    ...
    if(key >= '1' && key <= '3') toggleDisplayWireFrame(key - '1');
...
//    body.rigidBody.setDamping(0.1f, 0.1f);
  ...
  public void menger(float cx_, float cy_, float cz_, float dim_, int depth_, int max_depth){
    if(depth_ == max_depth){
      addCube(cx_, cy_, cz_, dim_, depth_, max_depth);
      return;
    }
    ...
    for(int z = -1; z <= 1; z++){
      for(int y = -1; y <= 1; y++){
        for(int x = -1; x <= 1; x++){
          ...
          float cy = cy_ + y * dim;
          float cz = cz_ + z * dim;
          int code = x*x + y*y + z*z;
          if(code <= 1){
            addCube(cx, cy, cz, dim, depth_, max_depth);
          } else {
            menger(cx, cy, cz, dim, depth, max_depth);
          }
        } 
      }
    }
  ...
  public void createFractureShape(){
    ...
    menger_color[0] = new int[]{4, 96, 255};
    menger_color[1] = new int[]{96, 255, 4};
    menger_color[2] = new int[]{255, 128, 4};
    ...
    cy = 0;
    cz = dim/2 + pos_z;
    menger(cx, cy, cz, dim, 0, 2);
    
    
    
//    menger_color[0] = new int[]{4, 96, 255};
//    menger_color[1] = new int[]{96, 255, 4};
//    menger_color[2] = new int[]{255, 255, 255};
//    
//    pos_z = 20f;
//    dim = 150;
//    cx = -dim * 0.60f;
...
//    
//    
//    menger_color[1] = new int[]{128, 64, 255};
//    menger_color[2] = new int[]{255, 255, 255};
//    menger_color[3] = new int[]{255, 128, 4};
...
//    pg_aa.save(file.getAbsolutePath());
  ...
  public static void main(String args[]) {
    PApplet.main(new String[] { Skylight_BulletPhysics_MengerSponge.class.getName() });
  }
}
