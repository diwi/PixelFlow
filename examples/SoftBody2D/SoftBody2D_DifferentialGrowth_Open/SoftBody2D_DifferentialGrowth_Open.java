...
package SoftBody2D.SoftBody2D_DifferentialGrowth_Open;
...
public class SoftBody2D_DifferentialGrowth_Open extends PApplet {
  ...
  public void setup() {
    ...
    param_chain.DAMP_VELOCITY  = 0.2999999f; 
...
//    initVersion_02();
  ...
  public void drawParticleLine(PGraphics2D pg, boolean display_particles, boolean display_line){
    ...
    if(display_particles){
      ...
      for(int i = 0; i < particles_sorted_count; i++){
        pcurr = particles_sorted[i];
        float radius = max(pcurr.rad * 1.1f, 5);
        float hue = i/(float)particles_sorted_count;
        pg.fill(hue, 1, 1);
  ...
  public static void main(String args[]) {
    PApplet.main(new String[] { SoftBody2D_DifferentialGrowth_Open.class.getName() });
  }
}